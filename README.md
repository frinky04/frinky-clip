...
- The window is the clip editor over the rolling buffer. The overview bar shows the whole history with footage, the marked range, and the viewed window; drag it to move. The lanes show a wall-clock ruler, keyframe thumbnails, and audio coverage: left-drag marks a range, drag a handle to adjust it, scroll to zoom, right-drag or drag the ruler to pan, and **Now** returns to the live edge. Handles snap to source frames. The strip above the lanes previews the exact In and Out frames. Footage in view or marked stays protected from expiry while the window is open.
...
Audio waveforms, a separate microphone track, stitching several ranges, hardware-accelerated thumbnail decoding, a bundled ffmpeg.exe, app-specific audio exclusion, HDR, Windows sign-in launch, and export while the recorder process is not running are future work. Thumbnails and preview frames decode AV1 in software: about 130 ms per keyframe and up to a second for an exact frame. Capture and encoding remain fixed at 1440p60. The latest clip remains accessible across recorder restarts while its file and status entry still exist.
