...
Clip editor concept: a clip is a range of the rolling buffer. Navigate with the overview (drag the viewport) and the lanes (drag to pan, scroll to zoom), mark in and out, and export a frame-accurate re-encode at the chosen resolution, frame rate, and bitrate to the clips folder. Lazy keyframe thumbnails and a cut-frame still stand in for a player. Segments under the viewed or marked range are protected from expiry while the editor uses them. Exports run in the recorder process with progress in the footer. "Save last N seconds" is the separate quick path and keeps recording quality. Per-clip microphone selection waits on a separate microphone track.
...
- Default to a 900 × 600 window, minimum 720 × 480, with the native title bar supplying the app name. The lanes take all spare height; the footer keeps a fixed height including one message line.
