...
Content plan: the clip editor is the work surface. A full-history overview bar sits above a zoomed lane view (ruler, video, audio; a microphone lane joins when a microphone track is recorded) with the clip range and export settings beneath it. The recorder lives in a filled footer strip that runs edge to edge along the bottom of the window, a different surface for a different subject: state, buffer clock, and a small disk gauge on the leading side; Record/Stop, quick save with its hotkey, and the clips folder on the trailing side; one message line beneath. The clip rows above it stay unboxed, directly under the lanes they describe, sharing the content's leading edge (Play) and trailing edge (Export). Capture, buffer, hotkey, app, and diagnostics live in a Settings overlay behind a single button. All controls remain native Dear ImGui widgets.

Clip editor concept: a clip is a range of the rolling buffer. Navigate with the overview (drag the viewport) and the lanes (drag to pan, scroll to zoom), mark in and out, and export a frame-accurate re-encode at the chosen resolution, frame rate, and bitrate to the clips folder. A conventional playhead drives a viewport above the lanes: click or scrub to place it, play with audio, and mark In and Out at it. Scrubbing shows the nearest cached keyframe under the mouse at once and lands the exact frame on release. Pause keeps the decoder and its queue warm so resume and forward steps are immediate. The filmstrip tiles keyframe thumbnails edge to edge across every run of footage at every zoom, so the lane reads as continuous footage rather than sparse samples. The video lane has a fixed height so thumbnails stay small and dense; spare height goes to the viewport. Segments under the viewed or marked range are protected from expiry while the editor uses them. Every session has one time base: segments chain by frame count from the session's start, so the player, the filmstrip, and the export place a frame at the same wall-clock time. Exports run in the recorder process through the bundled FFmpeg libraries with NVENC, H.264 by default and AV1 as an option, with progress in the footer. Ruler labels are wall-clock time so a moment stays put while recording; time-ago appears only in the header. "Save last N seconds" is the separate quick path and keeps recording quality. Per-clip microphone selection waits on a separate microphone track.
...
- Default to a window whose 16:9 viewport fills the content width, about 776 × 800 at 100% scaling, with the native title bar supplying the app name. The minimum keeps the viewport at its smallest usable height so nothing scrolls. The lanes have fixed heights and the viewport takes the spare height; the footer keeps a fixed height including one message line.
...
The app keeps no clip history; the player only plays the buffer. Folder opens the output location. Diagnostics lives at the bottom of Settings with logs and performance counters; errors and non-routine recorder messages show on the footer's message line. Omit explanatory text about standard window and tray behavior.
