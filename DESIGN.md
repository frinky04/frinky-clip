...
Content plan: the clip editor is the work surface. A full-history overview bar sits above a zoomed lane view (ruler, video, audio; a microphone lane joins when a microphone track is recorded) with the clip range and export settings beneath it. The recorder lives in a filled footer strip that runs edge to edge along the bottom of the window, a different surface for a different subject: state, buffer clock, and a small disk gauge on the leading side; Record/Stop, quick save with its hotkey, and the clips folder on the trailing side. Status lives on the action that started the work: Export fills with progress and then reads Exported, Save reads Saving and then Saved. Errors and notices are rare, so they earn a row of their own beneath, with Dismiss, and the strip shrinks back afterwards. The clip rows above it stay unboxed, directly under the lanes they describe, sharing the content's leading edge (Play) and trailing edge (Export). Capture, buffer, hotkey, app, and diagnostics live in a Settings overlay behind a single button. All controls remain native Dear ImGui widgets.
